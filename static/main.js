$(function(){
    ...
    async function getPlayers(term) {
        ...
        if(res.data.player){
            players = res.data.player.map( val => {
                return {label: val.strPlayer,
                    value: val.strPlayer,
                    category: "player",
                    dbid: val.idPlayer,
                    thumb: val.strThumb}
      ...
      $( "#follow-form #name" ).autocomplete({
        ...
        source: async function (request, response) {
            ...
            console.log(teams);
            let data = sports.concat(players, teams, leagues);
            response(data);
        },
        ...
        select: function(event, ui) {
            ...
            $('#follow-form #dbid').val(ui.item.dbid);
